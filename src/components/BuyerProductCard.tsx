...
export function BuyerProductCard({ product }: { product: BuyerProduct }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[1.35rem] border border-graphite/8 bg-white shadow-soft transition hover:-translate-y-1 hover:shadow-xl">
      <a className="block" href={`/kaskada-/products/${product.id}/`}>
        <div className="relative h-52 overflow-hidden bg-neutral-100">
          ...
          <div className="absolute left-3 top-3 flex flex-col gap-2">
            <span className="rounded-full bg-[#f6d44d] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-graphite">{product.badge}</span>
            {discount > 0 ? <span className="rounded-full bg-[#ff4d6d] px-3 py-1.5 text-[10px] font-black text-white">-{discount}%</span> : null}
          </div>
        </div>
      </a>
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-neutral-100 px-2.5 py-1.5 text-[11px] font-bold text-neutral-700"><HiStar className="text-gold" />{product.rating}</span>
        </div>
        ...
        <div className="mt-4 grid grid-cols-[44px_1fr] gap-2">
          <button className="flex h-11 items-center justify-center rounded-2xl border border-graphite/10 bg-white text-lg text-graphite" type="button" aria-label="В избранное"><HiHeart /></button>
          <button className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#ff4d6d] px-4 text-sm font-black text-white" type="button"><HiOutlineShoppingBag />В корзину</button>
        </div>
      </div>
    </article>
  );
