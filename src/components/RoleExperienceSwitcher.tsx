      ...
      <div className="card-glass reveal-up rounded-[1.75rem] p-6 shadow-soft lg:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold tracking-tight md:text-[2rem]">Интерфейс меняется в зависимости от роли</h2>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-neutral-600">Пока выбор доступен вручную. После авторизации платформа будет открывать нужный сценарий автоматически.</p>
          </div>
          <div className="grid grid-cols-2 gap-2 rounded-full border border-graphite/8 bg-white/76 p-1.5">
            {Object.entries(views).map(([key, item]) => (
              <button
                className={`rounded-full px-4 py-2.5 text-[13px] font-semibold transition ${role === key ? 'bg-graphite text-cream' : 'text-neutral-700 hover:bg-white'}`}
                key={key}
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="rounded-[1.4rem] border border-graphite/8 bg-white/76 p-5">
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-graphite text-2xl text-cream">{current.icon}</div>
