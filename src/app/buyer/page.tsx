    ...
    <main className="min-h-screen pb-24 text-graphite">
      <header className="sticky top-0 z-50 border-b border-graphite/8 bg-white/95 px-6 py-4 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-graphite/8 bg-[#f3f0ea] p-1.5 lg:flex">
            ...
            <a className="rounded-full px-4 py-2 text-sm font-bold hover:bg-white" href="#videos">Видео</a>
            <a className="rounded-full px-4 py-2 text-sm font-bold hover:bg-white" href="#brands">Бренды</a>
      ...
      <section className="mx-auto max-w-7xl px-6 py-6">
        <div className="hero-slider">
          {buyerHeroSlides.map((slide) => (
            <a className="relative block h-[390px] overflow-hidden rounded-[1.7rem] bg-graphite text-white" href={slide.href} key={slide.title}>
      ...
      <section className="mx-auto max-w-7xl px-6 py-4">
        <div className="grid gap-4 lg:grid-cols-2">
          <a className="relative overflow-hidden rounded-[1.6rem] bg-graphite text-white shadow-soft" href="#sale">
          ...
          <a className="relative overflow-hidden rounded-[1.6rem] bg-graphite text-white shadow-soft" href="#brands">
      ...
      <section className="mx-auto max-w-7xl px-6 py-8" id="videos">
        <div className="mb-5 flex items-end justify-between gap-4">
          ...
          <span className="hidden rounded-full bg-[#ff4d6d] px-4 py-2 text-xs font-black text-white md:inline-flex">Видео</span>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {buyerVideos.map((video) => (
            <article className="overflow-hidden rounded-[1.45rem] border border-graphite/8 bg-white shadow-soft" key={video.title}>
              <div className="relative h-56 overflow-hidden">
                <img alt={video.title} className="h-full w-full object-cover" src={video.image} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent" />
                <button className="absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white text-3xl text-[#ff4d6d] shadow-lg" type="button" aria-label="Запустить видео"><HiOutlinePlayCircle /></button>
                <span className="absolute bottom-4 right-4 rounded-full bg-black/70 px-3 py-1.5 text-[11px] font-bold text-white">{video.duration}</span>
              </div>
              <div className="p-4">
